...
configurenotify(XEvent *e)
{
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			//	XMoveResizeWindow(dpy, m->extrabarwin, m->wx+ sp, m->eby + vp+vp, m->ww-  2 * sp, bh);
				//  XMoveResizeWindow(dpy, m->extrabarwin, m->wx+ sp, m->eby+vp, m->ww- 2 * sp, bh);
			}
			focus(NULL);
			arrange(NULL);
...
//system("/home/yi/.dwm/autostart.sh &");
...
updatebarpos(Monitor *m)
{
	...
	if (m->showbar) {
		m->wh = m->wh - vertpad -vp;
		m->by = m->topbar ? m->wy - bh : m->wy +m->wh -vp ;
//		m->wy = m->topbar ? m->wy + bh + vp : m->wy;
		m->wy = m->topbar ? m->wy +  vp : m->wy;
		m->eby = m->topbar ? m->wy + m->wh  -vp : m->wy - bh;
	} else{
		m->by = -bh - vp;
		m->eby = -bh - vp;
	}
